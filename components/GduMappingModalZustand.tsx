...
interface GduMappingModalZustandProps {
...
const GduMappingModalZustand: React.FC<GduMappingModalZustandProps> = ({
  onConfirmMapping,
  ...
  const { 
    isGduMappingModalOpen,
    irrWorkflowState,
    closeGduMappingModal
  } = useIRRStore();
  
  const { semanticGduMapping, runA, runB } = irrWorkflowState;

  // Initialize user mappings with LLM proposals
  useEffect(() => {
    if (semanticGduMapping) {
      const initialMappings: Record<string, string | null> = {};
      semanticGduMapping.gdu_mappings.forEach(mapping => {
  ...
  if (!isGduMappingModalOpen || !semanticGduMapping || !runA || !runB) return null;
  ...
  const buildDisplayItems = (): GduMappingDisplayItem[] => {
    ...
    return semanticGduMapping.gdu_mappings.map(mapping => {
...
export default GduMappingModalZustand;