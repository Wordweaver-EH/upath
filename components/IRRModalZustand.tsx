...
interface IRRModalZustandProps {
...
const IRRModalZustand: React.FC<IRRModalZustandProps> = ({
  ...
  const handleFileLoad = (file: File, run: 'A' | 'B') => {
    updateIrrWorkflowState({ loadingState: 'loading-files' });
    ...
    reader.onload = (e) => {
      try {
        ...
        if (run === 'A') {
          updateIrrWorkflowState({ runA: parsedState, loadingState: 'idle' });
        } else {
          updateIrrWorkflowState({ runB: parsedState, loadingState: 'idle' });
        }
      } catch (error) {
        updateIrrWorkflowState({ 
          loadingState: 'error', 
...
export default IRRModalZustand;