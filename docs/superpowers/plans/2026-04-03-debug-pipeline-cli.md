# Debug Pipeline CLI Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a CLI script (`upath-backend/src/debug/run-pipeline.ts`) that runs any modularised pipeline step directly against the Gemini API, enabling fast prompt iteration without the Chrome UI.

**Architecture:** Single TypeScript file in `upath-backend/src/debug/`, run with `tsx`. Imports `generatePrompt`/`responseSchema` from existing frontend step configs via relative paths (`../../../src/config/pipeline/`). Calls Gemini directly using `@google/generative-ai`. P1.3 and P2S steps have dedicated runners due to their per-phase and per-DU iteration patterns. Outputs land in `upath-backend/debug-output/`.

**Tech Stack:** TypeScript, Node.js `util.parseArgs`, `@google/generative-ai` (already installed), `dotenv` (already installed), `tsx` (already installed). No new dependencies.

**Spec:** `docs/superpowers/specs/2026-04-03-debug-pipeline-cli-design.md`

---

### Task 1: Scaffold — file, npm script, tsconfig, gitignore

**Files:**
- Create: `upath-backend/src/debug/run-pipeline.ts`
- Modify: `upath-backend/package.json` (add `debug` script)
- Modify: `upath-backend/tsconfig.json` (exclude `src/debug` from build)
- Modify: `upath-backend/.gitignore` (create if missing, add `debug-output/`)

No tests for this task — this is a dev-only script. The backend's existing 10 tests must stay green throughout.

- [ ] **Step 1: Add `debug` script to `upath-backend/package.json`**

In `upath-backend/package.json`, change the `scripts` block to:
...
"scripts": {
  ...
  "test:ui": "vitest --ui",
  "test:run": "vitest run",
  "debug": "tsx src/debug/run-pipeline.ts"
},
```

- [ ] **Step 2: Exclude `src/debug` from TypeScript build**

In `upath-backend/tsconfig.json`, change the `exclude` array:

```json
"exclude": ["node_modules", "dist", "src/debug"]
```

This prevents `tsc` from complaining about imports that cross outside `rootDir` — the debug script is only ever run via `tsx`, not compiled.

- [ ] **Step 3: Create `upath-backend/debug-output/.gitkeep` and update `.gitignore`**

Create `upath-backend/.gitignore` (append if it already exists):
```
debug-output/
```

Create `upath-backend/debug-output/.gitkeep` (empty file so the directory is tracked by git but its contents are ignored).

- [ ] **Step 4: Create the scaffold file**

Create `upath-backend/src/debug/run-pipeline.ts` with all imports and an empty main that just prints a message:
...
- [ ] **Step 5: Verify the scaffold runs without import errors**

```bash
cd upath-backend && npm run debug
```

Expected output to stderr:
```
Debug pipeline CLI — scaffold only (not yet implemented)
```

If imports fail, check that relative paths reach `C:\Users\enigm\upath\src\config\pipeline\...` correctly. From `upath-backend/src/debug/`, three `../` steps reach the repo root, then `src/config/pipeline/...`.
...
Expected: 3 files, 10 tests (or current baseline), all pass.

- [ ] **Step 7: Commit**

```bash
cd upath-backend && git add src/debug/run-pipeline.ts package.json tsconfig.json .gitignore debug-output/.gitkeep
git commit -m "feat: scaffold debug pipeline CLI"
```

---

### Task 2: Core infrastructure — Gemini caller and step registry

**Files:**
- Modify: `upath-backend/src/debug/run-pipeline.ts`

- [ ] **Step 1: Add the Gemini caller function**

Add after the `DEBUG_OUTPUT_DIR` declaration:
...
- [ ] **Step 2: Add the step registry**

Add after the Gemini caller:
...
- [ ] **Step 3: Verify no TypeScript errors in the new code (run the scaffold again)**

```bash
cd upath-backend && npm run debug
```

Expected: same "scaffold only" message, no import or type errors.

- [ ] **Step 4: Commit**

```bash
git add upath-backend/src/debug/run-pipeline.ts
git commit -m "feat(debug-cli): add Gemini caller and step registry"
```

---

### Task 3: Step runners, file I/O, and summaries

**Files:**
- Modify: `upath-backend/src/debug/run-pipeline.ts`

- [ ] **Step 1: Add file I/O helpers**

Add after the step registry:
...
- [ ] **Step 3: Add the standard step runner**

Add after `printSummary`:
...
- [ ] **Step 4: Add the P1.3 special runner**

Add after `runStandardStep`:
...
async function runP13(p1_2_output: unknown, model: string): Promise<unknown> {
  ...
  for (const phaseName of P13_PHASES) {
    ...
    // P1.3 phases return JSON text (not structured JSON output mode)
...
- [ ] **Step 5: Verify the file still runs without errors**

```bash
cd upath-backend && npm run debug
```

Expected: "scaffold only" message, no errors.

- [ ] **Step 6: Commit**

```bash
git add upath-backend/src/debug/run-pipeline.ts
git commit -m "feat(debug-cli): add step runners, file I/O, and summaries"
```

---

### Task 4: Chain orchestration and argument parsing

**Files:**
- Modify: `upath-backend/src/debug/run-pipeline.ts`

- [ ] **Step 1: Add the step-range resolver**

Add after the P1.3 runner:
...
- [ ] **Step 2: Add the initial-input loader**

Add after `resolveStepRange`:
...
- [ ] **Step 3: Add the chain runner**

Add after `loadInitialInput`:
...
Note: `runP2S` is defined in Task 5. For now, calling it will cause a compile error. Leave it as a forward reference — `tsx` is lenient about declaration order within a file.

- [ ] **Step 4: Replace the `main` stub with the full argument parser and entry point**
...
- [ ] **Step 5: Smoke test — run p1_4 with a saved p1_3 output**

First obtain a real `p1_3_output.json` — either:
- Copy one from a previous Chrome UI session (save the P1.3 step output JSON from the browser's network tab or pipeline store)
- Or run the full chain from a transcript: `npm run debug -- --from p0_1 --transcript <file>`

Then test P1.4 in isolation:

```bash
cd upath-backend
echo '{"transcript_id":"test","sorted_segments":[],"independent_variable_details":"test","dependent_variable_focus":["awareness"]}' > debug-output/p1_3_output.json
npm run debug -- --step p1_4 --input debug-output/p1_3_output.json
```

Expected: stderr shows `▶ p1_4…`, DU list (may be empty for the stub input), saved path. No fatal errors.

Full test with real data (once a real p1_3_output.json is available):

```bash
npm run debug -- --step p1_4 --input debug-output/p1_3_output.json
```

Expected stderr: `[p1_4] N DUs:` followed by `du_1: ...`, `du_2: ...` etc.

- [ ] **Step 6: Commit**

```bash
git add upath-backend/src/debug/run-pipeline.ts
git commit -m "feat(debug-cli): add chain orchestration and arg parsing"
```

---

### Task 5: P2S per-DU runner

**Files:**
- Modify: `upath-backend/src/debug/run-pipeline.ts`

P2S steps are per-DU. P2S_1 needs P1.1 output (for segment text) and P1.4 output (for DU→segment ID mapping), loaded from `debug-output/`. P2S_2 and P2S_3 each operate on the previous P2S step's aggregate output.

- [ ] **Step 1: Add `runP2S` before `runChain`**

Insert this function before the `runChain` function added in Task 4:
...
- [ ] **Step 2: Verify the full file runs**

```bash
cd upath-backend && npm run debug
```

Expected: prints usage (no args provided), exits 1. No import or type errors.
...
- [ ] **Step 4: Smoke test P2S (optional — needs real P1.x outputs)**

If `debug-output/p1_1_output.json`, `p1_4_output.json`, and `p1_5_output.json` are available from a prior run:

```bash
npm run debug -- --step p2s_1 --input debug-output/p1_5_output.json
```

Expected stderr: `▶ p2s_1…`, per-DU progress lines, `[p2s_1] N DUs processed`, saved path.

- [ ] **Step 5: Commit**

```bash
git add upath-backend/src/debug/run-pipeline.ts
git commit -m "feat(debug-cli): add P2S per-DU runner, complete CLI implementation"
```
...
- [ ] **Step 1: Run the full diachronic chain on a real transcript**

Obtain a raw transcript (e.g., `p1s1.txt` from the transcripts directory). Run:

```bash
cd upath-backend
npm run debug -- --from p_neg1_1 --to p1_5 --transcript ../transcripts/p1s1.txt --dv-focus "awareness,attention"
```

Expected: six step outputs saved to `debug-output/`, stderr shows DU count for p1_4, phase count for p1_5. No fatal errors.

- [ ] **Step 2: Iterate on P1.4 in isolation**

```bash
# Edit prompt in: src/config/pipeline/part1/P1_4_diachronicUnitGrouping.ts
npm run debug -- --step p1_4 --input debug-output/p1_3_output.json
```

Expected: new p1_4 output with updated DU count/descriptions.
...
```bash
git add upath-backend/src/debug/run-pipeline.ts
git commit -m "chore(debug-cli): verify end-to-end and finalise implementation"
```
