...
import { ALL_PIPELINE_STEP_IDS_IN_ORDER } from '../config/pipelineConfig'
  ...
  subscribeWithSelector((set, get) => ({
    ...
    openHilModalWithContext: () => {
      ...
      if (currentStepInfo.inputData && (currentStepInfo.outputData || currentStepInfo.error)) {
        // Note: With LangGraph migration, prompt generation is handled by backend nodes
        // HIL functionality may be deprecated or needs backend integration
        const originalPrompt = `Step: ${currentStepInfo.stepId}\n(Prompt generated by LangGraph backend)`
    ...
    handleHilSubmit: async () => {
      ...
      // Note: With LangGraph migration, HIL feedback needs backend integration
      // For now, this creates a simplified meta-prompt
      const metaPrompt = `Step: ${stepInfo.stepId}
Previous response was problematic. User guidance for correction:
--- USER GUIDANCE START ---
${hilUserGuidance}
--- USER GUIDANCE END ---
...
Please provide a corrected response addressing the user's feedback.`
      ...
      // TODO: Integrate with LangGraph backend for HIL functionality
      set({
        hilContext: {
          ...hilContext,
          metaPrompt,
