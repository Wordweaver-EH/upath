...
- **Debug CLI — never use `--step p0_1`**: `--step p0_1` in isolation feeds p_neg1_1 output (IV/DV JSON) into P0.1 instead of the raw transcript, producing an empty `line_numbered_transcript`. This corrupts the debug-output cache. Always use `--from p_neg1_1 --to p0_1` or a longer chain to regenerate P0.1 output.
